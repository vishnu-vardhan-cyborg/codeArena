...
const loadProblemsFromSupabase = async (userId, problemId = "") => {
  ...
  if (problemError) {
    throw new Error(
      problemError.code === "PGRST205" || problemError.code === "42P01"
        ? "Run backend/schemas/problem-schema.sql in Supabase to create the problem catalog."
  ...
  if (!problemData?.length) {
    throw new Error(
      "The Supabase problem catalog is empty. Run backend/schemas/problem-schema.sql again to seed the problems."
    );
  }
